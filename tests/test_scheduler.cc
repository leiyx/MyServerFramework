 ...
 */
void test_fiber2() {
  LOG_INFO(g_logger) << "test_fiber2 begin";
  ...
  LOG_INFO(g_logger) << "test_fiber2 end";
}

void test_fiber3() {
  LOG_INFO(g_logger) << "test_fiber3 begin";
  LOG_INFO(g_logger) << "test_fiber3 end";
}

void test_fiber5() {
  static int count = 0;

  LOG_INFO(g_logger) << "test_fiber5 begin, i = " << count;
  LOG_INFO(g_logger) << "test_fiber5 end i = " << count;
...
void test_fiber4() {
  ...
  for (int i = 0; i < 3; i++) {
    serverframework::Scheduler::GetThis()->Schedule(
        test_fiber5, serverframework::GetThreadId());
  }
...
int main() {
   ...
   * 消耗掉，然后再返回main函数往下执行
   */
  serverframework::Scheduler sc;

  // 额外创建新的线程进行调度，那只要添加了调度任务，调度器马上就可以调度该任务
  // serverframework::Scheduler sc(3, false);
  ...
  serverframework::Fiber::ptr fiber(new serverframework::Fiber(&test_fiber3));
  sc.Schedule(fiber);
