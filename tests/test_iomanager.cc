...
void do_io_write() {
  ...
  if (so_err) {
    LOG_INFO(g_logger) << "connect fail";
    return;
  }
  LOG_INFO(g_logger) << "connect success";
}
...
void do_io_read() {
  ...
  if (readlen > 0) {
    buf[readlen] = '\0';
    LOG_INFO(g_logger) << "read " << readlen << " bytes, read: " << buf;
  } else if (readlen == 0) {
    LOG_INFO(g_logger) << "peer closed";
    close(sockfd);
    return;
  } else {
    LOG_ERROR(g_logger) << "err, errno=" << errno
                        << ", errstr=" << strerror(errno);
    close(sockfd);
    return;
  }
  // read之后重新添加读事件回调，这里不能直接调用addEvent，因为在当前位置fd的读事件上下文还是有效的，直接调用addEvent相当于重复添加相同事件
  serverframework::IOManager::GetThis()->Schedule(watch_io_read);
}

void watch_io_read() {
  LOG_INFO(g_logger) << "watch_io_read";
  serverframework::IOManager::GetThis()->AddEvent(
      sockfd, serverframework::IOManager::READ, do_io_read);
}
...
void test_io() {
  ...
  if (rt != 0) {
    if (errno == EINPROGRESS) {
      LOG_INFO(g_logger) << "EINPROGRESS";
      // 注册写事件回调，只用于判断connect是否成功
      // 非阻塞的TCP套接字connect一般无法立即建立连接，要通过套接字可写来判断connect是否已经成功
      serverframework::IOManager::GetThis()->AddEvent(
          sockfd, serverframework::IOManager::WRITE, do_io_write);
      // 注册读事件回调，注意事件是一次性的
      serverframework::IOManager::GetThis()->AddEvent(
          sockfd, serverframework::IOManager::READ, do_io_read);
    } else {
      LOG_ERROR(g_logger) << "connect error, errno:" << errno
                          << ", errstr:" << strerror(errno);
    }
  } else {
    LOG_ERROR(g_logger) << "else, errno:" << errno
                        << ", errstr:" << strerror(errno);
  }
}
...
void test_iomanager() {
  ...
  // serverframework::IOManager iom(10); // 演示多线程下IO协程在不同线程之间切换
